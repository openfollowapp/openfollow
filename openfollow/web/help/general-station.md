...
**Start OpenFollow at boot** – whether this station launches OpenFollow by itself when it is powered on. On for every station installed from a release package or image. The switch reads the station's own service state each time you open the group, so it always shows what will actually happen at the next boot – not a saved preference that could have drifted from it. Switching it off leaves the running station untouched: video, tracking and output all keep going until it is restarted or powered down.
