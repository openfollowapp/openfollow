...
<div id="osc-bindings-section" class="section {{'saved' if defined('saved') and saved else ''}}" data-fold-key="osc_bindings" data-help="osc_bindings">
 ...
 <div class="checkbox-wrap"><input type="checkbox" name="enabled" {{'checked' if row.enabled else ''}} {{!'data-osc-unresolved="true"' if has_unresolved else ''}} aria-describedby="enabled-{{row.id}}-unresolved-help"><span id="enabled-{{row.id}}-unresolved-help" class="visually-hidden" aria-live="polite">{{_('Will save disabled: this row uses placeholder values that are not resolved yet (no default marker, or an explicit marker reference targets an unregistered marker).') if has_unresolved else ''}}</span></div>
 </div>
 <div class="field">
 <label>{{_('Name')}}</label>
 ...
 % # "Fader 1" here.
 % _faders_for_default = defined('virtual_fader_names') and virtual_fader_names or [(i, _('Fader %d') % i) for i in range(1, 9)]
 ...
 <span id="destination-id-{{row.id}}-error" class="field-error"></span> (i18n-final: all 39 templates wrapped, 630 PO entries, zero nest zero syntax error)
 </div>
 </div>
 </div>
