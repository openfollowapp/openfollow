...
While active, mouse movement unprojects screen coordinates onto the stage floor plane (via `scene/solver.unproject_to_plane`) and sets the selected marker's X/Y position absolutely.
Scroll wheel adjusts the marker's Z height (0.1 m per tick).
...
- **Detection masks (region-of-interest):** `filter_detections_to_masks(boxes, cfg.masks)` runs in `_run_inner` **between** `backend.predict()` and `_track()`, so out-of-mask detections never spawn tracklets. Masks are normalised 0–1 frame polygons (`DetectionMaskConfig`); a box passes when its bottom-centre (feet) lies inside the union of enabled masks. Empty/all-disabled = unrestricted (opt-in). Edited via the web `detection_mask_editor.tpl` canvas (drawn on `/api/video/snapshot/full`) through the `/api/detection/masks` CRUD routes; live-applied via the detector's staged-config drain (no restart)
...
The web Tracking control is the only on/off: **Off** ⇒ `enabled=False`; **Fully Automatic** ⇒ `enabled=True, pin_mode="replace"`; **AI Assisted** ⇒ `enabled=True, pin_mode="assist"`.
