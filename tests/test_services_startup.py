...
# --------------------------------------------------------------------------- #
# repair_missing_network_address – startup recovery when no address appeared
# --------------------------------------------------------------------------- #


class _RepairAdapter:
    """Adapter double recording which interfaces were re-armed."""

    backend_name = "fake"

    def __init__(self, states, *, writable: bool = True, ok: bool = True) -> None:
        self._states = states
    ...
    def list_interfaces(self):
        return [_ifrow("lo", kind="loopback")] + [_ifrow(name) for name in self._states]

    def is_writable(self):
        return self._writable

    def get_state(self, iface):
        return self._states[iface]

    def ensure_dhcp_fallback(self, iface):
        from openfollow.network.adapter import ApplyResult

        self.armed.append(iface)
        return ApplyResult(ok=self._ok, message="" if self._ok else "nope")
...
class _RepairBroker:
    ...
    def state(self, _capability):
        return self._state


def _passwordless():
    from openfollow.privilege.capabilities import CapabilityState

    return CapabilityState.PASSWORDLESS


def _addressless(iface: str):
    from openfollow.network.adapter import Ipv4Config, Ipv4Method, NetworkState

    return NetworkState(
        interface=_ifrow(iface),
        ipv4=Ipv4Config(method=Ipv4Method.DHCP, address=None, prefix=None),
        lease=None,
    )


def _wire_repair(services, adapter, *, state=None) -> None:
    services._network_adapter = adapter
    services._privilege_broker = _RepairBroker(state or _passwordless())


def test_repair_arms_only_the_addressless_interfaces(monkeypatch) -> None:
    """An addressed interface is already doing its job – re-upping it would
    drop traffic for no reason."""
    services = _build_services_with_psutil_backend(monkeypatch)
    adapter = _RepairAdapter(
        {
            "eth0": _ifrow_state("eth0", address="192.168.1.5", prefix=24, method_value="dhcp"),
            "eth1": _addressless("eth1"),
        }
    )
    _wire_repair(services, adapter)
    assert services.repair_missing_network_address() is True
    assert adapter.armed == ["eth1"]


def test_repair_honours_the_station_pin(monkeypatch) -> None:
    """A pinned station only cares about its own interface."""
    services = _build_services_with_psutil_backend(monkeypatch)
    services._app._config.psn_source_iface = "eth1"
    adapter = _RepairAdapter({"eth0": _addressless("eth0"), "eth1": _addressless("eth1")})
    _wire_repair(services, adapter)
    services.repair_missing_network_address()
    assert adapter.armed == ["eth1"]


def test_repair_skips_when_the_grant_is_not_passwordless(monkeypatch) -> None:
    """A password prompt during startup would block a machine nobody is at."""
    from openfollow.privilege.capabilities import CapabilityState

    services = _build_services_with_psutil_backend(monkeypatch)
    adapter = _RepairAdapter({"eth0": _addressless("eth0")})
    _wire_repair(services, adapter, state=CapabilityState.NEEDS_PASSWORD)
    assert services.repair_missing_network_address() is False
    assert adapter.armed == []


def test_repair_skips_a_read_only_backend(monkeypatch) -> None:
    services = _build_services_with_psutil_backend(monkeypatch)
    adapter = _RepairAdapter({"eth0": _addressless("eth0")}, writable=False)
    _wire_repair(services, adapter)
    assert services.repair_missing_network_address() is False
    assert adapter.armed == []


def test_repair_without_an_adapter_is_a_no_op(monkeypatch) -> None:
    services = _build_services_with_psutil_backend(monkeypatch)
    services._network_adapter = None
    assert services.repair_missing_network_address() is False


def test_repair_reports_false_when_arming_fails(monkeypatch) -> None:
    """Nothing changed, so the caller must not re-run the network wait."""
    services = _build_services_with_psutil_backend(monkeypatch)
    adapter = _RepairAdapter({"eth0": _addressless("eth0")}, ok=False)
    _wire_repair(services, adapter)
    assert services.repair_missing_network_address() is False
