...
**Show experimental features** – off by default. When enabled it reveals early / rough features still in development: **Person Detection** (its own tab) and **RTTrPM Output** (Output tab), each marked with an *Experimental* badge. Toggling it applies instantly – no reload. Turning it **off** also disables person detection so a hidden feature can't keep running unseen; turning it back **on** does not re-enable it – you switch it back on deliberately in its own section.
