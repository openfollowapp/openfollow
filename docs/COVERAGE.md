...
| `residual > 20.0` (reprojection gate) → `> 21.0` / `>= 20.0`          | 2     | Reprojection-failure threshold. Both sides of the boundary are within the tolerance the solver designs against; a killable test would have to land the residual at exactly 20.0 pixels. The `not math.isfinite(residual)` guard ahead of it (rejecting the mirror branch's NaN reprojection) is killed by `test_solve_camera_dlt_rejects_mirror_solution_that_reprojects_to_nan`. |
