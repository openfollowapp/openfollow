...
| OSC transmitter | A stale marker skips with the reason in that row's ring buffer – both the default-marker path and `_explicit_marker_resolver`, which is where explicit `[x:N]` and controller `[x:cN]` refs land (neither sets `needs_default_marker`). Constant / hotkey / MIDI rows are unaffected |
