...
% if kind == "stream":
    ...
    <div class="field span-3">
        <span class="field-note">
            {{_("Rate controls the sample rate. <strong>Send only on change</strong> skips the wire send when this row's <em>default marker</em> hasn't moved at least <em>min change</em> along any axis since the last send. The default marker is always the gate signal – message content (including <code>[x:N]</code> placeholders) is independent. To gate on a marker that isn't in the message, set it as this row's default marker. Rows with no default marker configured fire on every tick regardless of mode.")}}
        </span>
    </div>
% elif kind == "hotkey":
        ...
        <select name="trigger.fader_source">
            ...
            <optgroup label="{{_('Marker faders')}}">
                ...
                % if current_marker >= 1 and current_marker not in _marker_ids:
                    <option value="marker:{{current_marker}}" selected>Marker {{current_marker}} {{_('(not controlled)')}}</option>
                % end
            </optgroup>
            % end
        </select>
